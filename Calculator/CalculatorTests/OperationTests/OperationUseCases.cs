...
namespace CalculatorTests.OperationTests
{
    ...
    {
        [Test]
        public void Run_ReturnsCorrectExecution([Range(-100, 100)] int x)
        {
            Assert.AreEqual(x + x, new Operation<int>((int x, int y) => x + y).Run(x, x));
        }
        
        [Test]
        public void Validator_Run_ReturnsCorrectExecution([Range(-100, 100)] int x)
        {
            Assert.AreEqual(x+x, new Operation<int>((int x) => x + x).AddValidator((int x)=>(true, string.Empty)).Run(x));
        }

        [Test]
        public void Formatter_Run_ReturnsCorrectExecution([Range(-100, 100)] int x)
        {

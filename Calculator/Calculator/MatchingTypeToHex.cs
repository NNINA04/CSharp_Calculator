...
namespace Calculator
{
    ...
    public class MatchingTypeToHex : IHexCalculator
    {
        ...
        /// Конструктор
        /// </summary>
        static MatchingTypeToHex()
        {
            _hexMatchings = GetMahingDictionary();
        }
        ...
        /// <returns>Hex значение</returns>
        public string ToHex(int x)
        {
            var bytes = BitConverter.GetBytes(x).ConvertArrayToBigEndian();

            StringBuilder sb = new(capacity: bytes.Length * 2 + bytes.Length - 1);

            //  Найдена ли пара байт, которая не равняется нулю 
            bool isFound = false;

            for (int i = 0; i < bytes.Length; i += 2)
            {
                if (isFound == false && bytes[i] + bytes[i + 1] == 0x0 && i < bytes.Length - 2)
                    continue;
                else
                    isFound = true;

                sb.Append(_hexMatchings[bytes[i]]);
                sb.Append(' ');
                sb.Append(_hexMatchings[bytes[i + 1]]);
        ...
        /// <summary>
        /// Возвращает Hex словарь
        /// </summary>
        /// <returns>Hex словарь, с соотношением байта и его hex представлением</returns>
        private static Dictionary<byte, string> GetMahingDictionary()
        {
            var hexMatсhings = new Dictionary<byte, string>
            {
